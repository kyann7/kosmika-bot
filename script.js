    ...
    update() {
        ...
        if (this.x < -100 || this.x > canvas.width + 100 || this.y < -100 || this.y > canvas.height + 100) {
            if (Math.random() < 0.005) {
                this.reset();
                if (Math.random() > 0.5) { this.x = Math.random() > 0.5 ? -50 : canvas.width + 50; } 
                else { this.y = Math.random() > 0.5 ? -50 : canvas.height + 50; }
            }
        }
    }
}
...
// ==========================================
// INJEÇÃO DOS CARDS DE STATUS REAIS
// ==========================================
...
async function atualizarDadosKosmika() {
    const SEU_GUILD_ID = "931659654369513542"; // O ID do seu servidor
    ...
    try {
        ...
        if (data && data.presence_count !== undefined) {
            // "presence_count" é a quantidade de pessoas ONLINE agora
            document.getElementById('membros-real').innerText = data.presence_count;
        }
    } catch (err) {
        console.log("Aguardando Widget sincronizar...");
        document.getElementById('membros-real').innerText = "1.2k"; // Fallback
    }
}

atualizarDadosKosmika();
// Atualiza os números a cada 30 segundos sem precisar dar F5
